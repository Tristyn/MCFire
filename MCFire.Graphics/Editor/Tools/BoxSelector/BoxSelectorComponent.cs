...
using MCFire.Graphics.Modules.BoxSelector.Messages;
...
namespace MCFire.Graphics.Modules.BoxSelector.Models
{
    ...
    public class BoxSelectorComponent : GameComponentBase
    {
        ...
        void DragStart(object sender, KeyEventArgs e)
        {
            ...
            // enumerate faces
            // left
            if (pos.X < roundedHit.X && Math.Abs((float) (hit.X - lesser.X)) < .01 && new Plane(lesser, Vector3.Left).Intersects(ref roundedHit) == PlaneIntersectionType.Intersecting)
                _faceDragRay = new Ray(roundedHit, Vector3.Left);
            // right
            else if (pos.X > roundedHit.X && Math.Abs((float) (hit.X - greater.X)) < .01 && new Plane(greater, Vector3.Right).Intersects(ref roundedHit) == PlaneIntersectionType.Intersecting)
                _faceDragRay = new Ray(roundedHit, Vector3.Right);
            // down
            else if (pos.Y < roundedHit.Y && Math.Abs((float) (hit.Y - lesser.Y)) < .01 && new Plane(lesser, Vector3.Down).Intersects(ref roundedHit) == PlaneIntersectionType.Intersecting)
                _faceDragRay = new Ray(roundedHit, Vector3.Down);
            // up
            else if (pos.Y > roundedHit.Y && Math.Abs((float) (hit.Y - greater.Y)) < .01 && new Plane(greater, Vector3.Up).Intersects(ref roundedHit) == PlaneIntersectionType.Intersecting)
                _faceDragRay = new Ray(roundedHit, Vector3.Up);
            // forward
            else if (pos.Z < roundedHit.Z && Math.Abs((float) (hit.Z - lesser.Z)) < .01 && new Plane(lesser, Vector3.ForwardRH).Intersects(ref roundedHit) == PlaneIntersectionType.Intersecting)
                _faceDragRay = new Ray(roundedHit, Vector3.ForwardRH);
            // back
            else if (pos.Z > roundedHit.Z && Math.Abs((float) (hit.Z - greater.Z)) < .01 && new Plane(greater, Vector3.BackwardRH).Intersects(ref roundedHit) == PlaneIntersectionType.Intersecting)
